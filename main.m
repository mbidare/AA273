...
yNoNoise = zeros(6,numSteps); yNoNoise = x0;
yGaussian = yNoNoise;
yUniform = yNoNoise;
yExp = yNoNoise;
yBrn = yNoNoise;
...
for i = 2:numSteps
    yNoNoise(:,i) = H*xNoNoise(:,i);
    yGaussian(:,i) = H*xGaussian(:,i) + gaussNoise(:,i);
    yUniform(:,i) = H*xUniform(:,i) + uniformNoise(:,i);
    yExp(:,i) = H*xExp(:,i) + expNoise(:,i); 
    yBrn(:,i) = H*xBrn(:,i) + brnNoise(:,i);
    yCauchy(:,i) = H*xCauchy(:,i) + cauchNoise(:,i); 
    yBiMod(:,i) = H*xBiMod(:,i) + biModalNoise(:,i);
end
